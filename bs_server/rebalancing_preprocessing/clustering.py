...
def cluster_louvain(cluster_df):
    for t in [60]:
        time_ind = math.floor(5 * 60 / t)
        # 调度量文件，两列，站点编号、调度量
        # 不需要调度的站点也需要放入文件中，调度量为0
        title = '动态调度量_08_'
        data = cluster_df.iloc[:, 1].to_numpy()
        del_list = []
        ...
        # 站点坐标
        Vor_XY = pd.read_excel('/Users/hurunqiu/project/bs_rebalancing_platform/bs_server/resources/dataset_dockless/lat_lon.xlsx').to_numpy()
        ...
        for i in range(length):
            for j in range(length):
                ...
                if Hour[i] * Hour[j] < 0 and dis < 2:
                    ...
                    file.write(
                        str(int(Vor_XY[i][0])) + ' ' + str(int(Vor_XY[j][0])) + ' ' + str(Weight[i][j]) + '\n')  #
        ...
        pd.DataFrame(center).to_csv(title + 'Louvain_center' + str(t) + '.csv', index=False)
        result = pd.DataFrame(
        ...
        plt.xticks(fontsize=48, fontproperties=font_en)
        plt.xlabel('经度', fontsize=28, fontproperties=font_cn)
        plt.yticks(fontsize=28, fontproperties=font_en)
        plt.ylabel('纬度', fontsize=48, fontproperties=font_cn)
        plt.savefig(title + 'Louvian_' + str(t) + '.png', dpi=600)
        plt.show()
